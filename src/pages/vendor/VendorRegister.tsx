...
import { ShieldCheck, Camera, FileText, Loader2, Store, User as UserIcon, Upload, X, Sparkles, Copy } from "lucide-react";
...
const VendorRegister = () => {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] grid place-items-center px-4 py-10 bg-primary">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <button
                  ...
                  className={cn(
                    ...
                    role === "customer" ? "border-success bg-success/5" : "border-border hover:border-foreground/20"
                  )}
                >
                  <UserIcon className="h-6 w-6 mb-2 text-accent" />
                  <div className="font-semibold">I'm a Customer</div>
                  <div className="text-xs text-muted-foreground">Buy and request repairs.</div>
                </button>
                <button
                  type="button"
                  ...
                  className={cn(
                    "p-4 rounded-xl border-2 text-left transition-smooth",
                    role === "vendor" ? "border-success bg-success/5" : "border-border hover:border-foreground/20"
                  )}
                >
                  <Store className="h-6 w-6 mb-2 text-accent" />
                  <div className="font-semibold">I'm a Vendor</div>
                  <div className="text-xs text-muted-foreground">Sell and offer repairs.</div>
                </button>
              </div>
              ...
              {role === "customer" ? (
                ...
                <form onSubmit={handleVendorSignUp} className="space-y-4">
                  ...
                  <Button type="submit" variant="success" size="lg" className="w-full" disabled={loading}>
                    {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Creating account…</> : "Create vendor account"}
                  </Button>
                </form>
              )}
            </TabsContent>
